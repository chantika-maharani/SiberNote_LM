...
export const registerUser = async(req: Request, res: Response) => {
    try {
        ...
        if (!email || !password || !username) {
            return response.userError(res, "email, username, dan password wajib diisi")
        }
        ...
        return response.requestSuccessWithData(res,"berhasil buat user",{token},201)
    } catch (error) {
...
export const loginUser = async (req:Request,res:Response)=>{
    try {
        ...
        if(!isMatch){
            return response.userError(res,"password salah")
        }
        const payload = {userId:user._id}
        const token = jwt.sign(payload,envVariable.JWT_KEY,{expiresIn:"1h"})
        
        return response.requestSuccessWithData(res,"berhasil login",{token},200)
    } catch (error) {
